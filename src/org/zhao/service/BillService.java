...
public interface BillService {
	...
	void calculateMonthDuration(String currYearMonth);
	
	/**根据service_detail表中的时长,计算费用**/
	void calculateCostAndDuration(List<ServiceDetail> serviceDetails);

	/**查询某个业务在某年某月份的登录详情信息**/
	List<ServiceDetail> findServiceDetailByBillMonth(String billYearMonth,Integer service_id);
	...
	List<BillItem> getBillItemInfoToCal(Integer serviceId,String billYearMonth);
	...
	/**根据账单id进行分页查找帐单条目**/
	List<Map<String, Object>> findBillItemsByBillIdPage(Integer billId, Page page);
	
	/**一个帐单项有多少回登录登出信息**/
	int findRowsFromServiceDetail(Integer itemId, Page page);
	
	/**根据帐单条目itemId进行分页查找账单业务详情**/
	List<ServiceDetail> findServiceDetailsByItemIdPage(Integer itemId, Page page);

	/**根据账单idbillId查找所有的账单条目**/
	List<BillItem> findBillItemsByBillId(Integer id);
	...
	DurationRank findDurationRankByAccountIdHostId(Integer account_id, Integer host_id);
